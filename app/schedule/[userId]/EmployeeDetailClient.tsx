                              ...
                              {sourceOts.length > 0 ? (
                                <div className="flex flex-col gap-1.5 items-start">
                                  {sourceOts.map((ot: any, idx: number) => (
                                    <span key={idx} className="inline-flex items-start gap-1.5 bg-orange-50 border border-orange-200 text-orange-700 px-2.5 py-1.5 rounded-md text-xs font-bold hover:bg-orange-100 transition-all w-full" onClick={(e) => { e.stopPropagation(); setSelectedOvertime(ot); setIsOvertimeModalOpen(true); }}>
                                      <Link2 className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                                      <span className="whitespace-normal break-keep leading-tight">{ot.title}</span>
                                    </span>
                                  ))}
                                </div>
