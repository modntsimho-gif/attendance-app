...
import { ArrowLeft, Calendar, Clock, CheckCircle2, XCircle, AlertCircle, ChevronRight } from "lucide-react";
import { useState } from "react";
// ⭐️ 두 모달 모두 import (경로 확인 필요)
...
export default function EmployeeDetailClient({ profile, leaves, overtimes }: EmployeeDetailClientProps) {
  ...
  // ⭐️ 초과근무 클릭 핸들러 (추가됨)
  const handleOvertimeClick = (ot: any) => {
              ...
              <div className="flex gap-4 w-full md:w-auto">
                <div className="flex-1 md:flex-none bg-blue-50 p-4 rounded-lg border border-blue-100 text-center min-w-[120px]">
                  ...
                  <div className="text-2xl font-bold text-gray-800">
                    ...
                    <span className="text-sm font-normal text-gray-400 ml-1">/ {Number(profile.total_leave_days).toFixed(2)}</span>
                  </div>
                </div>
                <div className="flex-1 md:flex-none bg-orange-50 p-4 rounded-lg border border-orange-100 text-center min-w-[120px]">
                  ...
                  <div className="text-2xl font-bold text-gray-800">
                    ...
                    <span className="text-sm font-normal text-gray-400 ml-1">/ {Number(profile.extra_leave_days).toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
                      ...
                      >
                        ...
                        <div className="flex justify-between items-end">
                          <div className="text-sm text-gray-600">
                            <div className="mb-1 truncate max-w-[200px]">{leave.reason}</div>
                      ...
                      return (
                        <div 
                          ...
                          className={`p-4 transition-colors cursor-pointer group ${
                            isFullyUsed ? 'bg-gray-50 opacity-70 hover:bg-gray-100' : 'bg-white hover:bg-orange-50/30'
                          }`}
                        >
                          <div className="flex justify-between items-start mb-1">
                            <div>
                              <div className="font-bold text-gray-800 text-sm flex items-center gap-2 group-hover:text-orange-700 transition-colors">
                                {ot.title}
