                      ...
                      groupedLeaves.map((group, groupIdx) => {
                        ...
                        return (
                          <React.Fragment key={groupIdx}>
                            {/* 최신 메인 행 */}
                            <tr 
                              className="hover:bg-blue-50/30 cursor-pointer transition-colors group/row"
                              onClick={() => handleLeaveClick(latest)}
                            >
                              <td className="px-4 py-3.5 text-center">{renderStatusBadge(latest.status)}</td>
                              <td className="px-4 py-3.5 font-medium text-gray-900">
                                <span className="bg-gray-100 text-gray-700 px-2 py-1 rounded text-xs">{latest.leave_type}</span>
                              </td>
                                ...
                                {sourceOvertimes.length > 0 ? (
                                  <div className="flex gap-1.5 flex-wrap">
                                    {sourceOvertimes.map((ot: any, i: number) => (
                                      <span 
                                        key={i} 
                                        className="inline-flex items-center gap-1.5 bg-orange-50 border border-orange-200 text-orange-700 px-2 py-1 rounded-md text-xs font-bold hover:bg-orange-100 hover:border-orange-300 transition-all shadow-sm cursor-pointer" 
                                        onClick={(e) => { 
                                          e.stopPropagation(); 
                            ...
                            {history.map((pastItem) => (
                              <tr 
                                key={pastItem.id} 
                                className="bg-gray-50/50 text-gray-400 text-xs hover:bg-gray-100/50 cursor-pointer transition-colors"
                                onClick={() => handleLeaveClick(pastItem)}
                              >
                                <td className="px-4 py-2 text-center opacity-60">{renderStatusBadge(pastItem.status)}</td>
                                <td className="px-4 py-2">
                                  <div className="flex items-center gap-1">
                                    <CornerDownRight className="w-3 h-3" />
                                    <span>{getRequestTypeLabel(pastItem.request_type)}</span>
                                  </div>
                                </td>
                        ...
                        return (
                          <React.Fragment key={groupIdx}>
                            ...
                            <tr 
                              ...
                              onClick={() => handleOvertimeClick(latest)}
                            >
                              <td className="px-4 py-3.5 text-center">{renderStatusBadge(latest.status)}</td>
                              <td className="px-4 py-3.5 font-bold text-gray-900">
                              ...
                              <td className="px-4 py-3.5 text-right">
                                {Number(latest.used_hours) > 0 ? (
                                  <span className="text-red-500 font-bold bg-red-50 px-2 py-0.5 rounded text-xs">
                                    -{Number(Number(latest.used_hours) / 8).toFixed(2)}일
                                  </span>
                                ) : <span className="text-gray-300">-</span>}
                              </td>
                              <td className="px-4 py-3.5 text-gray-700 truncate max-w-[350px]" title={latest.title}>
                                {latest.title}
                              </td>
                            </tr>
                            ...
                            {history.map((pastItem) => (
                              <tr 
                                key={pastItem.id} 
                                className="bg-gray-50/50 text-gray-400 text-xs hover:bg-gray-100/50 cursor-pointer transition-colors"
                                onClick={() => handleOvertimeClick(pastItem)}
                              >
                                <td className="px-4 py-2 text-center opacity-60">{renderStatusBadge(pastItem.status)}</td>
                                <td className="px-4 py-2">
                                  <div className="flex items-center gap-1">
